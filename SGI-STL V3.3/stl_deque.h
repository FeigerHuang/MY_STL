...
struct _Deque_iterator {
  ...
  _Tp* _M_cur;   // 此迭代器所指缓存区current元素
  _Tp* _M_first;  //此迭代器所指缓存区的头
  _Tp* _M_last;   //此迭代器所指缓存区的尾(含备用空间)
...
_Deque_base<_Tp,_Alloc>::_M_initialize_map(size_t __num_elements)
{
  ...
  size_t __num_nodes = 
    ...
    //最少分配 initial_map_size也就是 8 个节点;
  _M_map_size = max((size_t) _S_initial_map_size, __num_nodes + 2);
  _M_map = _M_allocate_map(_M_map_size);
    
    //设置nstart和nfinish指向map的中间;
  _Tp** __nstart = _M_map + (_M_map_size - __num_nodes) / 2;
  _Tp** __nfinish = __nstart + __num_nodes;
    //从nstart到nfinish创建缓冲区;
  __STL_TRY {
                                          ...
                                          bool __add_at_front)
{
  ...
  //如果加上一个节点后的节点个数小于 map总大小的一半;
  if (_M_map_size > 2 * __new_num_nodes) {
      //计算出新的start节点位置;
    __new_nstart = _M_map + (_M_map_size - __new_num_nodes) / 2 
                     + (__add_at_front ? __nodes_to_add : 0);
     // 如果 new_start 在 原起点之前;
    if (__new_nstart < _M_start._M_node)
      copy(_M_start._M_node, _M_finish._M_node + 1, __new_nstart);
    else
      copy_backward(_M_start._M_node, _M_finish._M_node + 1, 
                    __new_nstart + __old_num_nodes);
    //进行map上节点指针的copy_backward;
  }
  else {
   // 否则我们重新分配map;
